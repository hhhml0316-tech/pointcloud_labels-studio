...
- 上一帧、下一帧、连续播放、单框复制到相邻帧、整帧合并复制、撤销和重做
- 单框或整帧复制到相邻帧时自动检测 ID 不同但空间重叠的疑似重复目标，可逐对统一 ID 或保留
- 主视图文字标签重叠时，在同一位置再次点击可循环选中下层的框
- 右键编辑 Track ID 时可选“应用到整个序列”，把所有帧中的旧 ID 一并改名
...
5. 主视图仅通过“类别 + ID”文字标签选择对象；多个标签重叠时，在同一位置快速再次点击可循环切换选中。右键文字标签或右侧对象列表，可变更类别、跟踪标识或重新执行几何拟合。

右侧“整帧复制”可把当前帧全部框复制到上一帧或下一帧；选中单框后，右侧参数区也有“复制此框到上一帧/下一帧”，只推送选中的这一个框。两者共用同一套合并逻辑：按 Track ID 合并，目标帧中同 ID 的框会被当前帧的框更新，目标帧独有的框保留；复制会直接写入目标帧的标签文件。合并时还会做空间查重：若复制过去的框与目标帧保留的框 ID 不同但 BEV 上明显重叠（IoU ≥ 0.2 或中心距 ≤ 1 m），会弹窗让你逐对选择“统一为当前帧 ID”“统一为目标帧 ID”或“保留两个框”，避免同一目标出现两个不同 ID 的框。若某个目标的 ID 在多帧间不一致，可右键框选择“编辑跟踪标识”，勾选“应用到整个序列”后一次性把所有帧中的旧 ID 改为新 ID（同帧已存在新 ID 的帧会跳过并提示）。
